...
class Network: NSObject {
    ...
    var currentGameSquares = [String: String]()
    ...
    func updateCurrentGameSquaresListenerAndSingleton(game: String, completion: @escaping () -> Void) {
        ...
        currentGameSquaresListener = gamesCollectionRef.document(game).collection("squares").addSnapshotListener({ (squaresSnapshot, error) in
            ...
            if !squaresSnapshot.isEmpty {
                ...
                guard let square1 = squaresDocumentData["squares1"] as? String else { return }
                guard let square2 = squaresDocumentData["squares2"] as? String else { return }
                guard let square3 = squaresDocumentData["squares3"] as? String else { return }
                guard let square4 = squaresDocumentData["squares4"] as? String else { return }
                guard let square5 = squaresDocumentData["squares5"] as? String else { return }
                guard let square6 = squaresDocumentData["squares6"] as? String else { return }
                guard let square7 = squaresDocumentData["squares7"] as? String else { return }
                guard let square8 = squaresDocumentData["squares8"] as? String else { return }
                guard let square9 = squaresDocumentData["squares9"] as? String else { return }
    ...
    func updateSquare(game: String, square: String, symbol: String, completion: @escaping () -> Void) {
            ...
            if let error = error {
                ...
                self.gamesCollectionRef.document(game).collection("squares").document(documentID).updateData([square: symbol], completion: { (error) in
                    if let error = error {
                        ...
                        self.currentGameSquares[square] = symbol
                        NotificationCenter.default.post(name: NSNotification.Name(rawValue: "CurrentGameSquaresUpdated"), object: nil)
                        completion()
                    }
                })
